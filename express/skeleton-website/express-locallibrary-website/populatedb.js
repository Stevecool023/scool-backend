...
console.log(
  'This script populates some text books, authors, genres and bookinstances to the database. Specified database as argument - e.g.: node populatedb "mongodb+srv://cooluser:coolpassword@cluster0.lz91hw2.mongodb.net/local_library?retryWrites=true&w=majority"'
);
...
async function authorCreate(index, first_name, family_name, d_birth, d_death) {
  const authordetail = { first_name: first_name, family_name };
...
async function bookInstanceCreate(index, book, imprint, due_back, status) {
  ...
  if (due_back != false) bookinstancedetail.due_back;
...
async function createAuthors() {
  ...
  await Promise.all([
    ...
    authorCreate(2, "Isaac", "Asimov", "1932-01-02", "1992-04-06"),
...
async function createBooks() {
  ...
  await Promise.all([
    ...
    bookCreate(1,
      ...
      "Picking up the tale of Kvothe Kingkiller once again, we follow him into exile, into political intrigue, courtship, adventure, love and magic... and further along the path that has turned Kvothe, the mightiest magician of his age, a legend in his own time, into Kote, the uassuming pub landlord.",
      "9788401352836",
      authors[0],
      [genres[0]]
    ),
    ...
    bookCreate(3,
      "Apes and Angels",
      "HumanKind headed out to the stars not for conquest, nor exploration, nor even for curiosity. Human went to the stars in a desperate crusade to save intelligent life wherever they found it. A wave of death is spreading through the Milky Way galaxy, an expanding sphere of lethal gamma ...",
      "9780765379528",
      authors[1],
      [genres[1]]
    ),
